	...
	for (int row1 = 0; row1 < N; row1 += 1)
	{
		...
		for (int col2 = 0; col2 < N; col2 += 16)
		{
			...
			for (int col1 = 0; col1 < N; col1++)
			{
				...
				mSum1 = _mm_add_ps(mSum1, _mm_mul_ps(mMul0, mBstr1));
				mSum2 = _mm_add_ps(mSum2, _mm_mul_ps(mMul0, mBstr2));
				mSum3 = _mm_add_ps(mSum3, _mm_mul_ps(mMul0, mBstr3));
			}
			_mm_storeu_ps(&Mul[row1][col2], mSum0);
			_mm_storeu_ps(&Mul[row1][col2 + 4], mSum1);
			_mm_storeu_ps(&Mul[row1][col2 + 8], mSum2);
			_mm_storeu_ps(&Mul[row1][col2 + 12], mSum3);
		}
	}
	return Mul;
}
